...
HRESULT CMeshContainer::Initialize_Prototype(CModel::TYPE eModelType, const aiMesh * pAIMesh, CModel* pModel, _fmatrix PivotMatrix, Mesh* pMesh)
{
...
#pragma region VERTEXBUFFER
	...
	if (CModel::TYPE_NONANIM == eModelType)
		hr = Ready_Vertices(pAIMesh, PivotMatrix, pMesh);
	else 
		hr = Ready_AnimVertices(pAIMesh, pModel, pMesh);
...
#pragma region INDEXBUFFER
	...
	pMesh->mFaces = new Face[m_iNumPrimitives];
	for (_uint i = 0; i < m_iNumPrimitives; ++i)
	{
		...
		pMesh->mFaces[i].mIndices[0] = pAIMesh->mFaces[i].mIndices[0];
		pMesh->mFaces[i].mIndices[1] = pAIMesh->mFaces[i].mIndices[1];
		pMesh->mFaces[i].mIndices[2] = pAIMesh->mFaces[i].mIndices[2];
	}
...
HRESULT CMeshContainer::Ready_Vertices(const aiMesh* pAIMesh, _fmatrix PivotMatrix, Mesh* pMesh)
{
	...
	for (_uint i = 0; i < m_iNumVertices; ++i)
	{
		...
		memcpy(&pMesh->mVertices[i], &pAIMesh->mVertices[i], sizeof(_float3));
		...
		memcpy(&pMesh->mNormals[i], &pAIMesh->mNormals[i], sizeof(_float3));
		...
		memcpy(&pMesh->mTextureCoords[0][i], &pAIMesh->mTextureCoords[0][i], sizeof(_float2));
		memcpy(&pMesh->mTangents[i], &pAIMesh->mTangents[i], sizeof(_float2));
	}
...
HRESULT CMeshContainer::Ready_AnimVertices(const aiMesh* pAIMesh, CModel* pModel, Mesh* pMesh)
{
	...
	*pMesh->mTextureCoords = new XMFLOAT3[m_iNumVertices];
	...
	for (_uint i = 0; i < m_iNumVertices; ++i)
	{
		...
		memcpy(&pMesh->mNormals[i], &pAIMesh->mNormals[i], sizeof(_float3));
		memcpy(&pMesh->mTextureCoords[0][i], &pAIMesh->mTextureCoords[0][i], sizeof(_float2));
		memcpy(&pMesh->mTangents[i], &pAIMesh->mTangents[i], sizeof(_float3));
	}
	...
	for (_uint i = 0; i < pAIMesh->mNumBones; ++i)
	{
		...
		memset(pBone->mWeights, 0, sizeof(VertexWeight) * pBone->mNumWeights);
...
CMeshContainer * CMeshContainer::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, CModel::TYPE eModelType, const aiMesh * pAIMesh, CModel* pModel, _fmatrix PivotMatrix, Mesh* pMesh)
{
